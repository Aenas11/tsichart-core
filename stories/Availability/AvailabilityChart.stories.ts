...
6. **Warm Range**: Highlighted area indicates data stored in warm storage
    ...
    argTypes: {
        ...
        warmStoreDays: {
            ...
            description: 'Number of days for warm storage range (from current time backwards)',
        ...
        enableBrushLogging: {
            control: 'boolean',
            description: 'Enable console logging for brush move actions (demo purposes)',
            table: { defaultValue: { summary: 'true' } }
        },
        enableBrushEndLogging: {
            control: 'boolean',
            description: 'Enable console logging for brush end actions (demo purposes)',
            table: { defaultValue: { summary: 'true' } }
        }
    }
};
...
function renderAvailabilityChart(container: HTMLElement, options: any = {}) {
    ...
    try {
        ...
        const warmStoreRange = options.warmStoreEnabled !== false ? [
            new Date(Date.now() - (options.warmStoreDays || 3) * 24 * 60 * 60 * 1000).toISOString(),
        ...
        const brushMoveAction = options.enableBrushLogging !== false 
            ? (from, to) => {
                console.log('Brush Move Action:', {
                    ...
                    to: to.toISOString(),
                    duration: `${Math.round((to.getTime() - from.getTime()) / (1000 * 60))} minutes`
                });
            }
            : null;

        const brushMoveEndAction = options.enableBrushEndLogging !== false
...
function createAvailabilityChartStory(containerStyle: string) {
    return (args: any) => {
        const chartId = 'availability-chart-' + Math.random().toString(36).substring(7);
