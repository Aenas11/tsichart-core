...
`,
  ...
  argTypes: {
    ...
    keepSplitByColor: {
      control: "boolean",
      description:
        "Maintain consistent colors for split-by values across aggregates",
      table: { defaultValue: { summary: "false" } },
    },
  },
};
    ...
    productionLines.forEach((line) => {
      ...
      timestamps.forEach((timestamp) => {
        ...
        const lineMultiplier =
          line === "" ? 1 : 0.3 + (parseInt(line.split(" ")[1]) || 1) * 0.2;

        factoryData[factory][line][timestamp] = {
          Production:
            Math.round(
              (baseProduction + Math.random() * 40 - 20) * lineMultiplier * 10
            ) / 10,
  ...
  return stages.map((stage, stageIndex) => {
    ...
    lines.forEach((line) => {
      ...
      timestamps.forEach((timestamp) => {
        ...
        const lineMultiplier =
          line === "" ? 1.0 : line === "Line A" ? 0.8 : 0.6;

        // Ensure positive, non-zero values for proper D3.js scale calculations
        const finalValue = Math.max(
          15,
          Math.round((baseValue + variation) * lineMultiplier)
        );
        ...
        if (category === "" || category === "Target") {
          ...
          deptData[department][category][timestamp] = {
            ...
            Efficiency: Math.round(
              (85 + Math.random() * 15) * actualMultiplier
            ),
...
function renderBarChart(container: HTMLElement, options: any = {}) {
  ...
  try {
    ...
    if (!Array.isArray(factoryData) || factoryData.length === 0) {
      throw new Error(
        "Invalid data format: Expected non-empty array of aggregates"
      );
    }
    ...
    factoryData.forEach((aggregate, index) => {
      ...
      if (!aggKey) {
        throw new Error(
          `Invalid aggregate at index ${index}: Missing aggregate key`
        );
      }
      ...
      Object.keys(splitBys).forEach((splitBy) => {
        ...
        if (!timeData || typeof timeData !== "object") {
          throw new Error(
            `Invalid split-by ${splitBy} in ${aggKey}: Missing time data`
          );
        }
        ...
        Object.keys(timeData).forEach((timestamp) => {
          ...
          if (!measures || typeof measures !== "object") {
            throw new Error(
              `Invalid timestamp ${timestamp} in ${aggKey}/${splitBy}: Missing measures`
            );
          }
          ...
          Object.keys(measures).forEach((measureName) => {
            ...
            if (
              ...
              console.error(
                `Fixing invalid measure ${measureName} in ${aggKey}/${splitBy}/${timestamp}: was ${measureValue}, setting to 1`
              );
