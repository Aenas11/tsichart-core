...
const data = [
    {
        ...
        "__tsiColor__": "#1f77b4", 
        "__tsiAggIndex__": 0,
        "2023-01-01T00:00:00Z": {measures: {temperature: 21.5}, temperature: 22.5 },
        "2023-01-01T01:00:00Z": { measures: {temperature: 23.1}, temperature: 23.1 }
    }
];
    ...
    for (let sensorIndex = 0; sensorIndex < 3; sensorIndex++) {
        for (let roomIndex = 0; roomIndex < 2; roomIndex++) {
            ...
            for (let hourIndex = 0; hourIndex < 12; hourIndex++) {
                ...
                const tempValue = parseFloat((baseTemp + Math.sin(hourIndex / 3) * 3 + Math.random() * 2).toFixed(1));
                rowData[timestamp.toISOString()] = {
                    temperature: tempValue,
                    measures: { temperature: tempValue }
                };
            }
            data.push(rowData);
        }
    ...
    try {
        ...
        sampleData.forEach((rowData) => {
            ...
            chartComponentData.allTimestampsArray.forEach(timestamp => {
                const timestampData = rowData[timestamp];
                const rawValue = timestampData?.temperature ?? null;

                chartComponentData.timeArrays[aggKey][splitBy].push({
                    dateTime: new Date(timestamp),
                    temperature: rawValue,
                    measures: rawValue !== null ? { temperature: rawValue } : null
                });
            });
        });
        ...
        console.log(sampleData, gridOptions, aggregateExpressionOptions, chartComponentData, "sampleData, gridOptions, aggregateExpressionOptions, chartComponentData")
                ...
                container.innerHTML = `
                    <div style="padding: 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
                        ...
                        <div style="background: #e8f4f8; padding: 15px; border-radius: 4px; margin: 10px 0;">
                            ...
                            <p>These static methods are typically used by LineChart and other chart components to add grid functionality to their ellipsis menus following <strong>monolithic component</strong> patterns.</p>
                            <pre style="background: white; padding: 10px; border-radius: 4px; overflow-x: auto;"><code>// In LineChart.ts
const gridOption = Grid.createGridEllipsisOption(
    this.renderTarget, 
    this.chartOptions, 
    this.aggregateExpressionOptions, 
    this.chartComponentData
);
this.ellipsisMenu.addOption(gridOption);</code></pre>
                        </div>
                        ...
                        <div style="background: #fff3cd; padding: 15px; border-radius: 4px; margin: 10px 0;">
                            ...
                            <p>Charts can call <code>Grid.createGridEllipsisOption()</code> to add a "Display Grid" option to their control panels, providing seamless data table views that share the same data state.</p>
                        </div>
                        ...
                        <div style="background: #f0f8ff; padding: 15px; border-radius: 4px; margin: 10px 0;">
                            ...
                            <p>Each timestamp property should contain an object with the measure values:</p>
                            <pre style="background: white; padding: 10px; border-radius: 4px; overflow-x: auto;"><code>{
    "__tsiLabel__": "SensorA: Room1",
    "__tsiColor__": "#1f77b4",
    "2023-01-01T00:00:00Z": { measures: {temperature: 22.5}, temperature: 22.5 },
    "2023-01-01T01:00:00Z": { measures: {temperature: 23.1}, temperature: 23.1 }
}</code></pre>
                        </div>
                    </div>                            
