...
class EventsTableData {
    ...
    public setEvents(rawEvents, fromTsx, timeSeriesIdProperties, offset = null) {
        ...
        this.events = [];
        this.offsetName = offset !== null ? this.createOffsetName(offset) : null;
        rawEvents.forEach((rawEvent) => {
            let eventToUse = rawEvent;
            if (!fromTsx) {
                eventToUse = { ...rawEvent };
                if (eventToUse.timestamp && typeof eventToUse.timestamp === 'object' && 'value' in eventToUse.timestamp) {
                    eventToUse.timestamp = eventToUse.timestamp.value;
                }
                Object.keys(eventToUse).forEach(key => {
                    if (key !== 'timestamp' &&
                        typeof eventToUse[key] === 'object' &&
                        eventToUse[key] !== null &&
                        'value' in eventToUse[key]) {
                        eventToUse[key] = eventToUse[key].value;
                    }
                });
            }
            var event = new TimeSeriesEvent(eventToUse);
    ...
    public constructColumns() {
        ...
        this.events.forEach((event: TimeSeriesEvent) => {
            Object.keys(event.cells).forEach((cellKey: string) => {
                ...
                const columnName = cell.name ?? cell.key.split('_')[0] ?? cell.key;
