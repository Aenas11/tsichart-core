...
import { P as Pikaday } from './pikaday-B_NmGK0T.js';
...
var DateTimePicker = /** @class */ (function (_super) {
    ...
    DateTimePicker.prototype.render = function (chartOptions, minMillis, maxMillis, fromMillis, toMillis, onSet, onCancel) {
        ...
        var self = this;
        saveButtonContainer.append("button").classed("tsi-saveButton", true).text(this.getString("Save"))
            .on("click", function () {
            self.onSet(self.fromMillis, self.toMillis, self.chartOptions.offset, self.maxMillis === self.toMillis, self.getCurrentQuickTime());
            self.onSaveOrCancel();
        });
        saveButtonContainer.append('button')
    ...
    DateTimePicker.prototype.buildQuickTimesPanel = function () {
        ...
        var enteredQuickTimes = quickTimes.enter()
            ...
            .attr('aria-label', function (d) { return "".concat(_this.getString('select quick time of'), " ").concat(d[0]); });
        ...
        this.calendarPicker = new Pikaday({
            ...
            onDraw: function (d) {
                ...
                _this.calendar.select(".pika-single").selectAll(".pika-day")
                    ...
                    var date = new Date(Number(d3.select(this).attr("data-pika-year")), Number(d3.select(this).attr("data-pika-month")), Number(d3.select(this).attr("data-pika-day")));
    ...
    DateTimePicker.prototype.createTimePicker = function () {
        ...
        var createTimePicker = function (startOrEnd) {
            ...
            var timeLabel = fromOrToContainer.append("label")
                ...
                .attr('for', inputID)
                .attr('aria-label', "".concat(startOrEnd === 'start' ? _this.getString('Start time input') : _this.getString('End time input')))
                .text(_this.getString(startOrEnd));
            timeLabel.append("span")
                .classed("tsi-timeRequired", true)
                .text(_this.getString('*'));
