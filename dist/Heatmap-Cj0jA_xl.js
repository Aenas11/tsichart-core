...
import { L as Legend } from './Legend-E8mAStpf.js';
import { C as ChartComponent } from './ChartComponent-C6-wlPEA.js';
import { T as TemporalXAxisComponent } from './TemporalXAxisComponent-Da2bKDDh.js';
...
var HeatmapCanvas = /** @class */ (function (_super) {
    ...
    HeatmapCanvas.prototype.renderScale = function (aggColor, isOnlyAgg) {
        var _this = this;
        ...
        for (var i = 0; i <= 20; i++) {
            var color = this.getColor(aggColor, percentileCalc(i / 20), isOnlyAgg);
    ...
    HeatmapCanvas.prototype.getExtent = function () {
        var rawExtent = d3.extent(this.heatmapData.allValues);
        var extent = rawExtent;
            ...
            Object.keys(_this.heatmapData.timeValues[ts]).forEach(function (splitBy, sBI) {
                ...
                if (cellData != null) {
                    if (highlightedSplitBy && highlightedSplitBy != splitBy) {
                        _this.drawCell(cellData.rowI, cellData.colI, cellData.value, aggColor, isOnlyAgg, true);
                    }
                    else {
                        _this.drawCell(cellData.rowI, cellData.colI, cellData.value, aggColor, isOnlyAgg);
                    }
                }
            });
        });
    ...
    HeatmapCanvas.prototype.drawCell = function (rowI, colI, value, aggColor, isOnlyAgg, outOfFocus) {
    ...
    HeatmapCanvas.prototype.getColor = function (aggColor, index, isOnlyAgg) {
        var color;
        if (isOnlyAgg) {
            color = this.colorScale(index);
        }
        else {
            var interpolatedColorLuminance = this.colorScale(index);
