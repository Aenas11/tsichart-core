...
**TsiClient.ux** is a standalone module for data visualization and analytics for time series data. It can be used to build graphs and charts using generic JSON data.

**Note:** Azure Time Series Insights has been retired. This library now focuses exclusively on client-side visualization components and does not include server-side API functionality.
...
* [Additional References](#additional-references) for configuration settings and options
